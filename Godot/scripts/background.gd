...
func _process(_delta: float) -> void:
	...
	global_position = active_camera.get_screen_center_position()
	parallax_offset = global_position * VisualData.BACKGROUND_PARALLAX
	queue_redraw()

func _draw() -> void:
	draw_rect(Rect2(-5000, -5000, 10000, 10000), Color("050a14"), true)
	for star in stars:
		var wrapped := Vector2(fposmod(star.p.x - parallax_offset.x + 3000.0, 6000.0) - 3000.0, fposmod(star.p.y - parallax_offset.y + 2000.0, 4000.0) - 2000.0)
		draw_circle(wrapped, star.r, Color(0.6, 0.86, 1.0, star.a))
