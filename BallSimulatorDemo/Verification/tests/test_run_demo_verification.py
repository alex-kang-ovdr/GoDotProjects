...
from run_demo_verification import AVAILABLE_SUITES, build_framework_command, resolve_roots  # noqa: E402
...
class VerificationRunnerTests(unittest.TestCase):
    ...
    def test_builds_shared_framework_command(self) -> None:
        ...
        config_path = Path("D:/Demo/Tools/Testing/ProjectTests.json")
    ...
    def test_environment_overrides_take_precedence(self) -> None:
        previous_demo = os.environ.get("BALL_SIMULATOR_DEMO_ROOT")
        previous_framework = os.environ.get("GAME_TEST_FRAMEWORK_ROOT")
        os.environ["BALL_SIMULATOR_DEMO_ROOT"] = "D:/DemoOverride"
        os.environ["GAME_TEST_FRAMEWORK_ROOT"] = "D:/FrameworkOverride"
        try:
            demo_root, framework_root = resolve_roots()
            self.assertEqual(demo_root, Path("D:/DemoOverride").resolve())
            self.assertEqual(framework_root, Path("D:/FrameworkOverride").resolve())
        finally:
            if previous_demo is None:
                os.environ.pop("BALL_SIMULATOR_DEMO_ROOT", None)
            else:
                os.environ["BALL_SIMULATOR_DEMO_ROOT"] = previous_demo
