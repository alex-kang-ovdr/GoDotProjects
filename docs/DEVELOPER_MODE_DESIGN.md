...
## 파트 편집 그리드와 테스트 파일럿 미리보기

- 파트 편집 화면은 `Tree` 기반 7열 데이터 그리드(ID·이름·무기/기능·Hull·Mass·재질·설명)로 구성한다.
- 검색 입력은 ID·표시 이름·설명·재질·무기 타입을 부분 일치로 필터링하고, 카테고리 필터(전체·무기·추진·방어·탄약/기타)를 제공한다.
- ID·Hull·Mass·Type 정렬 버튼은 오름/내림차순을 토글한다. 정렬·필터는 CSV SSOT 값을 다시 읽지 않고 현재 메모리 행만 재배치해 즉시 반영한다.
- 행 선택 시 우측 `PartPreview`가 선택 스펙을 표시한다. 정지·추진 반복·공격 반복 모드에서 Hull/Mass/Thrust/RCS debug string과 추진·RCS·무기 방향 화살표/빔을 시각화한다.
- 미리보기는 편집 패널 전용 회색상자 시뮬레이션이며 실제 월드 물리 상태를 변경하지 않는다.

## 안전성/성능

개발자 UI는 CanvasLayer 한 개와 그리드/미리보기 컨트롤만 생성한다. 메뉴 중 물리·AI·스폰을 정지해 편집 화면에서 불필요한 시뮬레이션 비용과 상태 변이를 차단한다. 미리보기 `_process`는 선택된 패널에서만 실행하며, 런타임 CSV는 기존 로더/패키징 경로를 그대로 사용한다.
