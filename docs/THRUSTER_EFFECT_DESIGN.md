...
물리 힘 크기를 해당 모듈의 최대 힘으로 나눈 값을 이펙트 강도로 사용한다. 자동 제동을 포함하여 30% 미만은 효과를 숨긴다. 30%부터 최대 출력(100%)까지의 70% 구간을 균등한 14% 폭으로 5단계(1~5)로 나눈다.

- 1단계: 30~44% 미만 — 작은 불꽃과 최소 연기
- 2단계: 44~58% 미만 — 소형 효과
- 3단계: 58~72% 미만 — 중형 효과
- 4단계: 72~86% 미만 — 대형 효과
- 5단계: 86~100% — 최대 효과

각 단계는 불꽃/연기 수, 속도, 크기, 투명도를 함께 조절한다. 방출 기준 아래가 되면 emitter를 즉시 숨겨 남아 있던 연기까지 화면에 남지 않는다. 입력이 없는 자동 감속·자동 각속도 제동에도 동일한 모듈별 최대 추력 비율 기준을 적용한다. 자동 RCS는 각속도 0.35 rad/s에서만 최대 출력에 도달하며, 그 아래에서는 각속도에 비례해 힘과 이펙트 강도가 함께 낮아진다. 수동 A/D 입력은 항상 즉시 최대 출력이다.
