...
| `grade`, `grade_color` | 색상 등급과 표시 팔레트 | 장착 UI·개발자 미리보기·3D 발광/외곽선의 시각 메타데이터 |
| `design_theme`, `theme_code`, `themed_name` | 테란·인간 / 저그·생물 / 프로토스·하이테크 계열과 안정 이름 키 | 검색·필터·아트 재질·향후 테마별 텍스처 선택에 사용 |
...
등급·테마의 허용값, 네이밍 코드와 현재 분류 기준은 [파트 색상 등급 · 디자인 테마 분류](PART_STYLE_CLASSIFICATION.md)를 따른다. 등급은 수치 밸런스가 아니라 표시와 분류 전용 메타데이터다.
